...
impl PlotItem for Line<'_> {
    ...
    fn shapes(&self, _ui: &Ui, transform: &PlotTransform, shapes: &mut Vec<Shape>) {
            ...
            let mut draw_one_block = |xs: &[f64], ys: &[f64]| {
                ...
                if len >= 2 {
                        ...
                        for i in 0..(len - 1) {
                            ...
                            if *gradient_fill {
                                if let Some(grad) = gradient_color.as_ref() {
                                    fill_color =
                                        Rgba::from(grad(transform.value_from_position(p1)))
                                            .to_opaque()
                                            .multiply(fill_alpha)
                                            .into();
                                }
                    ...
                    let draw_stroke = final_stroke.width > 0.0
                        && final_stroke.color
                            != egui::epaint::ColorMode::Solid(Color32::TRANSPARENT);
                    if draw_stroke {
        ...
        if let Some(bx) = transform.broken_xaxis() {
            //draw a single contiguous chunk [i0..=i1]
            let mut draw_chunk = |i0: usize, i1: usize| {
                if i1 < i0 {
                    return;
                }
            ...
            for seg in &bx.segments {
                ...
                for i in 0..len {
                    ...
                    if inside {
                        ...
                        // flush previous run s0..last_in
                        draw_chunk(s0, last_in);
                    }
                }
    ...
    fn geometry(&self) -> PlotGeometry<'_> {
        if let Some(b) = &self.blocks_xy {
            PlotGeometry::BlocksXY {
                xs_blocks: b.xs.clone(),
                ys_blocks: b.ys.clone(),
            }
